...
export interface SharedEmbedOptions {
     ...
     * Any fields referenced in this filter are expected to be allowed for filtering in the "Embed Chart" for each chart you wish to filter on or in the "Embed Dashboard" dialog if you are applying a filter on a dashboard.
     *
     * If filtering is required on a dynamic field defined via a Lookup, Calculated or Query Bar, then `filter` is appropriate property to set.
     */
    preFilter?: PlainObject;
}
/**
...
export type EmbedOptions = EmbedChartOptions | EmbedDashboardOptions;
export type EventHandlerPayload = Record<string, unknown>;
export type EventHandlerOptions = {
    includes?: object[];
};
export type EmbedChartEvent = 'click' | 'refresh';
export type EventHandler = (payload: EventHandlerPayload) => void;
export type EventSender = new (...args: any[]) => {
    /**
